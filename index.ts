...
const REST_APP_ID = process.env.REST_APP_ID,
  ...
  SHOW_PAREMETERS_IN_LOG = process.env.SHOW_PAREMETERS_IN_LOG || false,
...
exports.main = async function(context, req) {
  ...
  if (
    ...
    if (
      ...
      console.log(
        'Always run triggered - will run any time function is triggered.'
      )
      credentials = await msRestNodeAuth.loginWithServicePrincipalSecret(
        REST_APP_ID,
        REST_APP_SECRET,
...
class AddLoadBalancerPort {
  ...
  public async getLoadBalancer() {
    if (!this.loadBalancerJSON) {
      ...
      try {
        ...
        throw console.error(
          'Error in getting Load Balancer Data from Azure: ' + err
        )
      }
    } else {
  ...
  public getMappedProtocol(fortigateProtocol): Models.TransportProtocol {
    if (fortigateProtocol === 'tcp') {
      ...
      console.log(
        'Unkown protocol found ' + fortigateProtocol + ' returning null'
      )
      return null
    }
  }
    ...
    if (PERSISTENCE.toLowerCase() === 'default') {
      ...
      throw console.error(
        'No protocol could be maped using the current values:' +
          PERSISTENCE +
          ' Values must be one of the following Default" | "SourceIP" | "SourceIPProtocol'
      )
    }
  }
  ...
  public async getFrontEndPublicIP() {
    ...
    if (getELB && getELB.frontendIPConfigurations) {
      ...
      throw console.error(
        'Error in getFrontEndPublicIP. JSON data could not be retrieved.'
      )
    }
  ...
  public async getProbePort() {
    ...
    if (getELB && getELB.probes) {
      ...
      throw console.error(
        'Error in getProbePort. Probes data could not be retrieved.'
      )
    }
    // Throw an error here or else typescript will complain
    throw console.error(
  ...
  public async buildLoadBalancerParameters() {
    ...
    try {
      ...
      console.log(
        `Error fetching JSON List in buildLoadBalancerParameters : ${err}`
      )
      throw err
    }
    ...
    if (vipJSONList && vipJSONList.results) {
      ...
      for (let vipList of vipJSONList.results) {
        if (
          parseInt(vipList.extport, 10) === 0 ||
          parseInt(vipList.mappedport, 10) === 0
        ) {
          console.log(
            'External and Backend Ports of 0 are not supported. (Make sure PortForwarding is enabled). Skipping Rule: ' +
              vipList.name
          )
          ...
          for (var port in getRange) {
            ...
            if (
              mappedProtocol === 'Tcp' &&
              portsAddedTCP.includes(getRange[port])
            ) {
              console.log(
